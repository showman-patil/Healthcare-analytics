...
export default function PatientNotifications() {
  ...
  const typeConfig = {
    critical: { icon: AlertTriangle, bg: "bg-red-50 border-red-200", iconBg: "bg-red-100", iconColor: "text-red-600", badge: "bg-red-100 text-red-700" },
    warning: { icon: AlertTriangle, bg: "bg-yellow-50 border-yellow-200", iconBg: "bg-yellow-100", iconColor: "text-yellow-600", badge: "bg-yellow-100 text-yellow-700" },
    info: { icon: Info, bg: "bg-blue-50 border-blue-200", iconBg: "bg-blue-100", iconColor: "text-blue-600", badge: "bg-blue-100 text-blue-700" },
    success: { icon: CheckCircle, bg: "bg-green-50 border-green-200", iconBg: "bg-green-100", iconColor: "text-green-600", badge: "bg-green-100 text-green-700" },
  };
    ...
    <div ref={container} className="space-y-6">
      <div className="gsap-in flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="mt-1 text-muted-foreground">{unreadCount} unread alerts</p>
        </div>
        <button
      ...
      <div className="gsap-in space-y-3">
        ...
        {filtered.map((notification) => {
          ...
          return (
            <div
              key={notification.id}
              className={`flex gap-4 rounded-2xl border p-4 transition-all ${config.bg} ${!notification.read ? "shadow-sm" : "opacity-75"}`}
            >
